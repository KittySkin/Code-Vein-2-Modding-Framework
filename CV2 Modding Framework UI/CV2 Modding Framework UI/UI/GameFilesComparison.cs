...
public partial class GameFilesComparison : Form
{
    ...
    private void StartComparisonButton_Click(object sender, EventArgs e)
    {
        ...
        {
            MessageBox.Show(@"Please select both old and new game files before starting comparison.", @"Error",
                MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
        }

        string scriptPath = Path.Combine(Application.StartupPath, Utils.Constants.HelperScriptsFolder,
            Utils.Constants.CompareGameFilesBathod);

        ProcessStartInfo processStartInfo = new ProcessStartInfo
        {
            FileName = "cmd.exe",
            Arguments =
                $"/c \"\"{scriptPath}\" \"{selectOldGameFilesButton.Text}\" \"{SelectNewGameFilesButton.Text}\"\"",
        ...
        try
        {
            ...
            {
                MessageBox.Show($@"Comparison completed with errors. Exit code: {comparesionProcess?.ExitCode}",
                    @"Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                string logPath = Path.Combine(Application.StartupPath, Utils.Constants.HelperScriptsFolder,
        ...
        {
            MessageBox.Show($@"Failed to start comparison: {ex.Message}", @"Error", MessageBoxButtons.OK,
